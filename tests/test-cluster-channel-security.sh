...
echo "── Человеческие имена НЕ переименовываются ──"
# Санитайз обязан снимать только разделители пути. Если он тронет пробелы или
# кириллицу, имя файла кэша сменится, старый файл останется — а читаются они
# глобом `peers/*.stats`, то есть нода будет посчитана дважды: двойной трафик и
# двойной онлайн. Это дороже любой косметики в имени.
for human in 'Finnish Noda 2' 'Герм-1' 'node-b.example.net' 'Нода №2 (юг)'; do
    got=$(_cluster_safe_name "$human")
    [ "$got" = "$human" ] && ok "сохранено как есть: $human" \
                          || bad "имя изменено: «$human» -> «$got» (осиротит кэш)"
done
