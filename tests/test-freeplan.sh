...
# --- P-131: оплатившего снимают с тарифа и на ноде БЕЗ своего free=1 ---------
# Гейт free_enabled стоял первым в freeplan_tick, поэтому уборка за оплатившими
# на такую ноду не доходила. Строка висела вечно, и клиент видел в приложении
# «квота исчерпана» при полностью работающем платном доступе, а нода с free=1
# при следующем прогоне отключала его за перерасход.
freeplan_set user3 active 100 100 0 100 0 0 100
freeplan_has user3 || fail "подготовка: user3 должен быть на бесплатном"
EXPIRY=$(date -d '+30 days' +%Y-%m-%d)          # снова оплатил
free_enabled && fail "подготовка: на этой ноде free=1 быть не должно"

freeplan_tick

freeplan_has user3 && fail "оплатившего не сняли с бесплатного тарифа (нода без free=1)"

# А тот, у кого срок действительно истёк, с тарифа не слетает.
freeplan_set user4 active 100 100 0 100 0 0 100
EXPIRY=$(date -d '5 days ago' +%Y-%m-%d)
freeplan_tick
freeplan_has user4 || fail "юзера с истёкшим сроком сняли с бесплатного тарифа"

echo "✅ freeplan: окна, лимиты, пороги и выход по оплате ок"
