...
# --- P-20: два опроса одной оплаты параллельно (крон + кнопка) ---
# Фулфилмент длинный (в бою ~18 с: провижининг с рестартом протоколов), а след
# в журнале появляется в самом конце. Без замка второй проход всё это время
# видит «не выдавали» и продлевает второй раз.
l6=$(ym_new_label)
ym_pending_add "$l6" 781 m1 dvoyka "1030.93" 1000
ym_find_operation() { echo "op-3 1000.01"; }
bot_fulfill_payment() { sleep 2; printf '%s|%s|u|c|%s|%s|%s\n' "$(date '+%F %T')" "$2" "$4" "$5" "$6" >> "$PAYMENTS_LOG"; }
ym_settle "$l6" & ym_settle "$l6" &
wait
n=$(grep -c '|ym:op-3$' "$PAYMENTS_LOG")
[ "$n" = 1 ] || fail "двойное зачисление: выдач по одной операции — $n"
