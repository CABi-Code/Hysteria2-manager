...
# В режиме HY2M_MANAGER_ONLY ничего не спрашиваем, и терминала быть не должно:
# сюда приходят из systemd-юнита, крона и кластерного крючка, где /dev/tty нет
# вовсе. Требование терминала обрывало такое обновление на первой же строке.
if [ "${HY2M_MANAGER_ONLY:-}" != 1 ] && [ ! -t 0 ]; then
    if [ -e /dev/tty ]; then
        exec </dev/tty
    else
...
if [ "$HYSTERIA_PRESENT" = 1 ]; then
    # Неинтерактивный режим для обновления по кластерному крючку и из крона.
    # Разрешён ТОЛЬКО manager_only, и это не экономия на флаге, а граница:
    # полная переустановка стирает пользователей, конфиг и сертификаты, поэтому
    # запустить её без человека за клавиатурой нельзя ни одним значением
    # переменной. Хочешь переустановить — заходи руками.
    if [ "${HY2M_MANAGER_ONLY:-}" = 1 ]; then
        MODE="manager_only"
        echo "🔍 Неинтерактивное обновление: только менеджер."
    else
