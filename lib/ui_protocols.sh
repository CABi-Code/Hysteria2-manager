        ...
        case "$choice" in
            ...
            7)
                ...
                if ! proto_any_enabled; then
                    ...
                    if proto_bootstrap; then
                        # Ключи в подписке несут sni/pbk/sid/fp/порт ВНУТРИ себя.
                        # Пересобрать конфиг ноды и не перевыпустить ссылки —
                        # значит развести сервер и клиентов: сервер уже ждёт
                        # новое имя, а в ссылках у людей старое, и VLESS у них
                        # не поднимается. Само это лечилось только ближайшей
                        # cluster_sync, то есть до пяти минут молчаливого «не
                        # работает» ровно после того, как админ всё сделал верно.
                        echo "  ⏳ Перевыпуск ссылок в подписках..."
                        sub_refresh
                        echo "  ✅ Готово. Клиенты получат новые ключи при обновлении подписки."
                    else
                        echo "  ⚠️  Были ошибки — см. вывод выше"
                    fi
                fi
                pause
                ;;
