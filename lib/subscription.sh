...
# Фильтр протоколов для конкретного юзера (stdin → stdout). Сейчас режет TUIC
# у демо-профилей: sing-box не отдаёт юзера в API, пер-юзерного учёта трафика
# там нет — квоту демо не проверить, и безлимитный TUIC-ключ стал бы идеальной
# мишенью для фарма. Полноскоростной TUIC — только после оплаты.
