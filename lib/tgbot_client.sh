...
# реферальная ссылка ведёт на /start бота. См. cibpn-webapp/docs/BOT-START.md.
...
# доставка нажатия — long-polling наш. См. cibpn-webapp/docs/WEB-LOGIN.md.
...
bot_fulfill_payment() {   # chat_id tg_id payload total_amount currency charge_id [tg_handle]
    ...
    # докупаются отдельно и за деньги (надстройка, cibpn-webapp/docs/DEVICES.md),
