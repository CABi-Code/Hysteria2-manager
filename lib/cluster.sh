...
# POST к пиру тем же секретом. Отдельно от cluster_call: тот тянет данные
# (GET по статике Caddy), а этот дёргает ручку Web API пира.
cluster_post() {   # host path [timeout] -> stdout
    local host="$1" path="$2" to="${3:-5}" secret
    secret=$(cluster_secret)
    curl -fsS -X POST --max-time "$to" -H "X-Cluster-Auth: $secret" \
        "https://${host}/api${path}" 2>/dev/null
}

# ---- Крючки «сделай сейчас» ---------------------------------------------
# Обмен в кластере построен на том, что каждая нода ЗАБИРАЕТ данные своим
# кроном (~5 мин). Для денег это слишком долго: человек оплатил и сразу открыл
# кабинет, а профиль на соседях ещё не заведён — он видит один сервер вместо
# всех и считает, что купил неработающее. Крючок ничего не передаёт, он лишь
# говорит соседу «синхронизируйся сейчас», и тот забирает всё сам — прежним
# путём, с прежними проверками.
#
# Best-effort ПО ЗАМЫСЛУ: нода старой версии ответит 404, лежачая — ничем, и
# оба случая штатны, потому что крон остаётся страховкой. Поэтому в фоне и
# молча: ставить оплату в зависимость от доступности соседа нельзя.
cluster_nudge_peers() {
    sub_enabled || return 0
    local host
    while read -r host; do
        [ -n "$host" ] || continue
        ( cluster_post "$host" "/v1/cluster/nudge" 5 >/dev/null 2>&1 & ) 2>/dev/null
    done < <(cluster_peers)
    return 0
}
...
cluster_update_peers() {
    sub_enabled || return 0
    local host ok=0 fail=0
    while read -r host; do
        [ -n "$host" ] || continue
        if cluster_post "$host" "/v1/cluster/update" 20 >/dev/null 2>&1; then
            echo "  💚 $host — обновление запущено"
            ok=$((ok + 1))
        else
            echo "  ❌ $host — не принял (старая версия без ручки, недоступен или другой секрет)"
            fail=$((fail + 1))
        fi
    done < <(cluster_peers)
...
cluster_stats_live() {
    ...
    # Свою статистику здесь НЕ публикуем (P-46). Интерактивной сессии нужны ЧУЖИЕ
    # данные, а свои она писала за компанию — и в этом была беда: bash разбирает
    # lib/*.sh один раз при старте, поэтому открытое меню продолжает писать файл
    # кодом на момент запуска и раз в пару секунд затирает то, что публикуют
    # свежие кроны. После обновления это выглядит как «правка не работает».
    # Публикация и так идёт раз в минуту из cluster_online_sync.
...
# nosync — не ходить по пирам ЗА данными: публикация локальная и мгновенная, а
# полный обход занимает секунды, которых нет в ответе клиенту (провижининг из
# API/бота). Но соседей мы всё равно будим крючком: без него они узнают о
# покупке только на своём кроне, и человек до пяти минут видит в подписке одну
# ноду вместо всех. Крючок ничего не ждёт и ничего не гарантирует — данные
# сосед забирает сам, крон остаётся страховкой.
cluster_share_user() {   # user [nosync]
    ...
    publish_roster
    publish_cluster_state
    [ "${2:-}" = nosync ] && { cluster_nudge_peers; return 0; }
    cluster_sync
}
...
publish_cluster_userlimits() {
    sub_enabled || return 0
    mkdir -p "$WEBROOT/cluster"
    local tmp="$WEBROOT/cluster/userlimits.tmp.$BASHPID" u d h t r p
    : > "$tmp"
    while IFS= read -r u; do
        [ -n "$u" ] || continue
        ...
        p=$(get_user_prefer "$u")
        # rate — поле 5, prefer — поле 6 (ts остаётся полем 4 ради обратной
        # совместимости старых нод: они читают первые четыре и лишние игнорируют).
        printf '%s|%s|%s|%s|%s|%s\n' "$u" "$d" "$h" "$t" "$r" "$p" >> "$tmp"
    done < <(cluster_users_all)
...
cluster_apply_userlimits() {
    ...
    merged=$(
        ...
        | awk -F'|' 'NF>=4 && $1!="" { if (($4+0) > (ts[$1]+0)) { ts[$1]=$4; d[$1]=$2; h[$1]=$3; r[$1]=($5==""?0:$5); p[$1]=$6 } }
                     END { for (u in ts) printf "%s|%s|%s|%s|%s|%s\n", u, d[u], h[u], ts[u], r[u], p[u] }'
    )
    [ -n "$merged" ] || return 0
    local u d h t r p localts changed=0
    while IFS='|' read -r u d h t r p; do
        [ -n "$u" ] || continue
        [[ "$u" =~ ^[a-zA-Z0-9_-]+$ ]] || continue
        db_user_exists "$u" || is_user_disabled "$u" || continue   # юзера тут нет — не наше дело
        localts=$(userlimits_get_ts "$u")
        [ "${t:-0}" -gt "${localts:-0}" ] 2>/dev/null || continue
        set_user_limits "$u" "$d" "$h" "$t" "$r" "$p"
        changed=1
    done <<< "$merged"
    if [ "$changed" = 1 ]; then
        write_authlimits
