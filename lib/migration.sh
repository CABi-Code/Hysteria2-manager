...
# Проверка пары user:pass по базе. Число устройств здесь по-прежнему НЕ
# считается: резать по числу сессий на auth ломало переподключения и переезд
# между нодами. Отказать может только занятый СЛОТ (ниже) — он привязан к
# конкретному креду, поэтому своё же переподключение под него не попадает.
...
' "$DB" || {
    ...
    [ -r "$SLOTS" ] || exit 1
    slot=$(awk -F'|' -v u="$user" -v p="$pass" '$1==u && $2==p { print $3; exit }' "$SLOTS")
    [ -n "$slot" ] || exit 1
}
...
# Слот занят, если его помнят за ДРУГИМ адресом и тот адрес всё ещё в сети по
# последнему снимку (onlineips.dat, раз в минуту). Свой адрес пускается всегда:
# переподключения и параллельные сессии одного устройства слот не расходуют.
if [ -n "$ip" ] && [ -r "$SLOTMAP" ] && [ -r "$ONLINEIPS" ] \
   && grep -q '^SLOT_REJECT=1' "$NODECONF" 2>/dev/null; then
    prev=$(awk -F'|' -v u="$user" -v s="$slot" '$1==u && $2==s { last=$3 } END { print last }' "$SLOTMAP")
    if [ -n "$prev" ] && [ "$prev" != "$ip" ] && grep -qxF "${user}|${prev}" "$ONLINEIPS"; then
        exit 1
    fi
fi
