...
AUTHEOF
    ...
    chown root:hysteria "$AUTH_SCRIPT" 2>/dev/null || true
...
migrate_to_command_auth() {
    ...
    # Самопроверка скрипта на первой паре из базы — чтобы не сломать вход всем.
    # Если не прошла, конфиг НЕ трогаем и сервер НЕ перезапускаем.
    local tu tp
    IFS=: read -r tu tp < "$USERS_DB"
    if [ -n "$tu" ] && ! "$AUTH_SCRIPT" "127.0.0.1:0" "${tu}:${tp}" "0" >/dev/null 2>&1; then
