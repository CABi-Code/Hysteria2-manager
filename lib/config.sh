...
# Каденс тика. Был 5 с — чтобы спидометр мини-аппа получал плавный ряд выборок
# (идея 17: стрелка идёт по таймлайну, а не прыгает раз в 15 с). Цена оценивалась
# в ~9% ядра; замер на боевой ноде 1 vCPU показал **~19%** — второй потребитель
# после крона (P-119). Тик — не «один проход awk»: это отдельный процесс, который
# сорсит все модули и опрашивает API протоколов, ~1000 форков на прогон.
# 15 с возвращают ноде ~13% ядра; ряд выборок спидометр сглаживает сам.
# Увеличьте ещё, если нода тесная. МЕЖНОДОВЫЙ обмен скоростью от этого НЕ
# страдает — см. RATES_SYNC_EVERY.
RATES_TICK_SEC="${RATES_TICK_SEC:-15}"
# Раз во сколько тиков стягивать скорость пиров (cluster_rates_sync). Делитель
# держит межнодовый gossip на ~15 с независимо от локального каденса: при тике 5 с
# он был 3, при 15 с достаточно 1. Кластеру ничего не прибавилось — те же ~15 с,
# что и раньше. Свою скорость публикуем в любом случае: cluster_rates_sync
# начинается с publish_rates, а на негейченном тике его зовут отдельно.
RATES_SYNC_EVERY="${RATES_SYNC_EVERY:-1}"
# Пересчёт «по запросу» (webapi verb traffic-refresh): страница веб-аппа просит
# свежие цифры чаще, чем тикают таймеры. Чаще этого не пересчитываем — опрос
# API протоколов стоит ~0.5 c, а один пересчёт всё равно общий для всех, кто
# спросил. Файл метки — TRAFFIC_REFRESH_TS.
TRAFFIC_REFRESH_MIN_SEC="${TRAFFIC_REFRESH_MIN_SEC:-3}"
TRAFFIC_REFRESH_TS="$DATA_DIR/traffic_refresh.ts"
...
SLOTMAP_FILE="$DATA_DIR/slotmap.dat"       # «user|слот|ip|ts» — кто занял слот (пишет auth-скрипт)
...
# Просьба «обновите менеджер», объявляемая по ФАЙЛОВОМУ каналу: одна строка
# «версия|ts». Раньше это был POST в Web API соседа — и обновление кластера
# зависело от компонента, который на ноде без кабинета просто не включают
# (P-133). Файловый канал обязан работать, иначе кластера нет вообще, поэтому
# доставка кода теперь опирается только на него.
# Наш файл содержит ТОЛЬКО нашу просьбу: чужие мы не перепубликовываем, иначе
# объявление стало бы бессмертным (правило 3 в docs/guide/CLUSTER-SCOPE.md).
UPDATEREQ_FILE="$DATA_DIR/cluster_updatereq.dat"
# Когда последний раз ПЫТАЛИСЬ обновиться по просьбе (unix-время). Триггер —
# сверка по факту («просят версию новее нашей»), поэтому он остаётся верным,
# пока дело не сделано, и без паузы между попытками превратился бы в
# переустановку на каждой синхронизации. Хранить «этот ts я видел» вместо
# попытки нельзя: не доехавшее обновление не повторилось бы никогда — правило 7
# в docs/guide/CLUSTER-SCOPE.md.
UPDATEREQ_SEEN_FILE="$DATA_DIR/.cluster_updatereq.seen"
UPDATEREQ_RETRY_SEC="${UPDATEREQ_RETRY_SEC:-1800}"   # пауза между попытками
UPDATEREQ_TTL_SEC="${UPDATEREQ_TTL_SEC:-86400}"      # после этого просьба протухла
...
# Чужие вирт-хосты (мини-апп, редирект основного домена). Менеджер
