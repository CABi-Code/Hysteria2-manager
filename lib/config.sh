...
# Каденс тика. 5 с — чтобы спидометр мини-аппа получал плавный ряд выборок
# (идея 17: стрелка идёт по таймлайну, а не прыгает раз в 15 с). ~9% ядра на
# ноде (доминируют вызовы API протоколов, тик — один проход awk). Увеличьте,
# если нода тесная: спидометр сгладит разницу. МЕЖНОДОВЫЙ обмен скоростью НЕ
# учащается — он гейчится RATES_SYNC_EVERY (см. --rates-tick).
RATES_TICK_SEC="${RATES_TICK_SEC:-5}"
# Раз во сколько тиков стягивать скорость пиров (cluster_rates_sync). При тике 5 с
# и делителе 3 межнодовый gossip остаётся ~15 с, как раньше — локальный спидометр
# частит, кластер не грузим. Локальный collect_rates идёт каждый тик.
RATES_SYNC_EVERY="${RATES_SYNC_EVERY:-3}"
...
ONLINEIPS_FILE="$DATA_DIR/onlineips.dat"   # «user|ip» — живые адреса, снимок для auth-скрипта
...
# Чужие вирт-хосты (мини-апп cibpn-webapp, редирект основного домена). Менеджер
