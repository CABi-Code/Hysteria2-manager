...
# Токены юзера ПО ВСЕМУ КЛАСТЕРУ (локальные + кэш пиров). Это список того, что
# нода ОБЯЗАНА обслуживать (подписки, пароли слотов, заголовки Caddy), а не
# список устройств: см. sub_links_used.
sub_tokens_cluster() {
    { sub_tokens_all "$1"
...
# Доп. токены ПИРОВ: в файле каждой ноды ПЕРВАЯ строка юзера — её собственный
# основной токен (свой основной заводит каждая нода, см. regen_user_subscription).
# Это копии одной и той же подписки, а не купленные ссылки, и считать их за
# устройства нельзя: в кластере из N нод человек терял бы N−1 оплаченных
# устройств и не мог бы добавить то, за что заплатил (P-101).
sub_peer_extra_tokens() {   # user
    [ -d "$PEERS_DIR" ] || return 0
    local f
    for f in "$PEERS_DIR"/*.subtokens; do
        [ -f "$f" ] || continue
        awk -F: -v u="$1" '$1==u && ++n > 1 {print $2}' "$f" 2>/dev/null
    done
}

# Сколько ссылок-устройств у юзера занято: свои (основная + доп.) плюс доп.
# ссылки, выданные другими нодами.
sub_links_used() {   # user
    { sub_tokens_all "$1"; sub_peer_extra_tokens "$1"; } | grep -c .
}
...
# Лимит устройств упал — лишние ссылки обязаны умереть вместе с ним (P-100).
# Ссылка проверяется на лимит ОДИН раз, при выдаче (sub_link_add), и дальше
# живёт сама по себе. Пока лимит не резал токены, проданное или отобранное
# устройство продолжало работать по уже розданной ссылке ровно столько, сколько
# человек её хранит: свой слот, свой пароль, никакой проверки после выдачи.
#
# Режем СВОИ доп. токены, начиная с самых новых: основной не трогаем никогда
# (человек остаётся без подписки), чужие принадлежат выдавшей их ноде — она
# подрежет их у себя, когда до неё дойдёт новый лимит (запись лимита кластерная).
sub_links_prune() {   # user -> сколько ссылок снято
    local user="$1" allowed tok gone=0
    allowed=$(sub_links_allowed "$user")
    [ "${allowed:-0}" -gt 0 ] 2>/dev/null || { echo 0; return 0; }   # 0 = без лимита
    while [ "$(sub_links_used "$user")" -gt "$allowed" ]; do
        tok=$(sub_tokens_all "$user" | tail -n1)
        [ -n "$tok" ] || break
        sub_link_remove "$user" "$tok" || break     # 2 = дошли до основного, резать нечего
        gone=$((gone + 1))
    done
    # Пароль слота обязан умереть вместе со ссылкой: файл подписки уже удалён,
    # но у устройства ключ давно сохранён локально, и пускает его именно этот
    # справочник. Ждать общего sub_refresh нельзя — это ещё минута доступа.
    [ "$gone" -gt 0 ] && write_slotpass_db
    echo "$gone"
}
...
# Предпочитаемый ключ юзера — первой строкой (stdin → stdout, остальной порядок
# сохраняется). Клиент выбирает сервер позицией в подписке, а не в приложении:
# часть клиентов (проверено на Happ) при каждом включении берёт верхний ключ и
# выбор пользователя не помнит (P-76). Совпадение — по «host/схема»: host это то,
# по чему кабинет и так группирует ключи по серверам (parse_direct_link в
# webapi/wa_dispatch.py), а схема отличает VLESS от Hysteria2 на одной ноде.
# Предпочтения нет или ключ исчез (нода отвалилась, протокол выключен) — список
# идёт как собрался, без ошибки.
sub_prefer_sort() {   # user
    local pref=""
    declare -F get_user_prefer >/dev/null 2>&1 && pref=$(get_user_prefer "$1")
    [ -n "$pref" ] || { cat; return 0; }
    awk -v pref="$pref" '
        {
          scheme = tolower(substr($0, 1, index($0, "://") - 1))
          s = $0
          sub(/^[^/]*\/\//, "", s)     # убрать схему
          sub(/[/?#].*/, "", s)        # оставить user:pass@host:port
          n = split(s, p, "@"); hp = p[n]   # host:port — после ПОСЛЕДНЕГО @
          sub(/:.*/, "", hp)           # порт не различает ключи одного сервера
          if (!hit && (hp "/" scheme) == pref) hit = $0; else rest[++k] = $0
        }
        END { if (hit) print hit; for (i = 1; i <= k; i++) print rest[i] }'
}
