...
collect_rates() {
    ...
    printf '#label|%s\n' "$(node_label)" > "${RATES_FILE}.tmp.$BASHPID"
    ...
    printf '%s\n' "$merged" | awk -F'|' -v now="$now" -v prevf="$RATES_PREV_FILE" \
        -v rates="${RATES_FILE}.tmp.$BASHPID" -v prevout="${RATES_PREV_FILE}.tmp.$BASHPID" '
        BEGIN {
    ...
    mv "${RATES_FILE}.tmp.$BASHPID" "$RATES_FILE" 2>/dev/null
    mv "${RATES_PREV_FILE}.tmp.$BASHPID" "$RATES_PREV_FILE" 2>/dev/null
}

# (Мгновенную скорость юзера из RATES_FILE читает Python-webapi напрямую; здесь
# bash-обёртки НЕТ намеренно: имя get_user_rate занято тарифом из lib/limits.sh —
# два разных смысла под одним именем зависели бы от порядка source и молча ломали
# бы тарифный шейпинг при перестановке модулей.)
