...
if [ "$1" = "--collect" ]; then
    ...
    # Перевыпущенный Caddy серт разложить по Trojan/TUIC/Hysteria2 (раз в ~60
    # дней; в остальные разы это две cmp и выход). Подписки перевыпустит
    # --cluster-sync, он ходит чаще.
    proto_sync_certs
