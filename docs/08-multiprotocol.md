...
1. **Побайтный трафик TUIC** (sing-box) считается ПРИБЛИЖЁННО
   (`proto_collect_tuic_traffic`, раз в минуту): у sing-box 1.13 нет StatsService
   (`v2ray_api` убрали в 1.8), поэтому суммируем дельты байт соединений из
