...
уведомления о балансе — в cibpn-webapp (`docs/NOTIFICATIONS.md` там же).
...
- **`config:cache` в cibpn-webapp**: тесты при закэшированном конфиге стирают боевую БД
  (см. cibpn-webapp `docs/`), к уведомлениям прямого отношения нет, но помните.
