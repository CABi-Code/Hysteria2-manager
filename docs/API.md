...
### POST /v1/users/{name}/refresh — пересчитать и отдать (scope: read)

Тот же ответ, что `GET /v1/users/{name}`, плюс поле `refreshed`, но перед
сборкой профиля менеджер **пересчитывает трафик и активность прямо сейчас**
(`collect_activity`), не дожидаясь своего минутного тика. Для живых индикаторов:
шкала лимита демо-ключа и «в сети» на публичной главной веб-аппа обновляются в
такт миганию индикатора.

Ничего не меняет — только освежает снимок, поэтому scope `read`. Пересчёт
**глобальный и с кулдауном** `TRAFFIC_REFRESH_MIN_SEC` (по умолчанию 3 с) и с
неблокирующим локом: если недавно уже считали или занята другая мутация, ответ
приходит мгновенно с `refreshed:false` — данные при этом всё равно настолько
свежие, насколько возможно. Так что частый опрос упирается в кулдаун ноды, а не
нагружает её: десяти спросившим подряд хватает одного прогона.

Цена реального пересчёта — опрос API всех протоколов, ~1–3 с (доминируют Xray
statsquery и TUIC). Поэтому кулдаун не стоит опускать ниже пары секунд.

```bash
curl -X POST -H "$H" "$BASE/v1/users/alice/refresh"
```
