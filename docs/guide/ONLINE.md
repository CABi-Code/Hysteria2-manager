...
- `collect_rates` (`lib/traffic.sh`): один опрос кумулятива монотонных протоколов
  ...
  это цена опроса API протоколов. Счётчик уменьшился (сбор сделал `?clear=1` /
  `-reset`) → весь текущий кумулятив набежал уже ПОСЛЕ сброса, он и есть дельта
  за интервал. Раньше в этом случае писался 0, и спидометр проваливался на
  каждом сборе — а сбор идёт в том числе из TUI, на каждой перерисовке (P-35).
