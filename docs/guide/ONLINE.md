...
Xray — `api statsonlineiplist -email` по каждому юзеру (`statsquery` счётчики
онлайна не отдаёт), считаются только IP свежее `XRAY_ONLINE_WINDOW_SEC` (120 с):
готовое число `statsonline` брать нельзя — это размер карты онлайн-IP, которую
Xray не чистит, и мобильный юзер за сутки набирает там 5–11 «устройств» (P-33).
TUIC — соединения `clash_api`, разложенные по юзерам тем же резолвом по
`sourceIP`, что и активность. См. P-15/P-17/P-33 в docs/issues/PROTOCOLS.md.
