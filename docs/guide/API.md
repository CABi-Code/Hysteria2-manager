...
### GET /v1/users/by-subtoken/{token} — владелец ссылки-подписки (scope: read)

То же тело, что `/v1/users/{name}`. Нужно, чтобы показать человеку страницу
подписки, когда ссылку открыли браузером — см.
[SUB-BROWSER.md](SUB-BROWSER.md). Поиск идёт и по токенам, выданным соседними
нодами. `404 unknown_token` — такого токена нет ни у кого;
`400 invalid_token` — формат не тот.
...
### POST /v1/cluster/nudge · /update — крючки между нодами (только ноды)

Единственные ручки, куда **не пускают по ключу приложения вообще** — даже со
`*` в scopes. Авторизация только заголовком `X-Cluster-Auth` с общим секретом
кластера: это разговор нод между собой, а не ручка кабинета. Без заголовка —
`401`, с ключом приложения — `403`.

| Ручка | Что делает |
|---|---|
| `POST /v1/cluster/nudge` | «синхронизируйся сейчас» — нода идёт и забирает данные сама, не дожидаясь своего крона (~5 мин) |
| `POST /v1/cluster/update` | «обнови у себя менеджер» — ставится **только менеджер**, данные, конфиг, сертификаты и Hysteria не трогаются |

Обе отвечают сразу (`{"accepted": "1"}`) и делают работу в фоне: синхронизация
идёт минуты, а обновление перезапускает сам этот демон.

`nudge` ничего не передаёт — он лишь будит соседа, и тот забирает данные прежним
путём с прежними проверками. Best-effort по замыслу: нода старой версии ответит
`404`, лежачая — ничем, и оба случая штатны, потому что крон остаётся
страховкой.

`update` берёт адрес источника из конфигурации самой ноды, а не из запроса,
поэтому «обнови себя» не может превратиться в «выполни мой код». Полная
переустановка удалённо недоступна в принципе — она стирает пользователей.

```bash
curl -X POST -H "X-Cluster-Auth: <секрет кластера>" "$BASE/v1/cluster/nudge"
```

Из менеджера: `./hy2-manager.sh --cluster-update` — попросить обновиться все
соседние ноды; `--self-update` — обновить эту.
