...
Система распределена по двум сторонам: подписка и напоминания — здесь,
в менеджере; уведомления о движении денег шлёт **надстройка** — внешний
потребитель Web API (свой репозиторий, своя документация).
...
| Напоминания об истечении (7д/3д/1д/1ч/10мин) | **менеджер** `lib/notify.sh` | cron `--notify-sweep` каждые 5 мин | ⭐ |
...
| Автопродление не прошло (не хватило баланса) | **надстройка** | последний день подписки | ❗️ |
| Живой лог операции для АДМИНА (одно редактируемое сообщение) | **надстройка** | создание счёта → шаги оплаты → зачисление | 💳 🛒 |
| Зачисление на баланс | **надстройка** | факт зачисления денег | 💵 |
| Списание с баланса | **надстройка** | факт списания | 🪙 |

Каждое уведомление доставляется **ровно в один чат** (см. §5), приоритет —
личка бота:
1. **личка бота** — если Telegram её принял (диалог с ботом начат и не заблокирован);
2. **«от лица канала»** — только если личка НЕ приняла: в персональный чат прямых
   сообщений канала, со ссылкой на бота вместо кнопок. Работает при выполненных
   предусловиях (§5), иначе человек не получает ничего — это норма.

Раньше слали в оба канала сразу, и владелец обоих чатов получал каждое сообщение
дважды ([issues/OPS.md#p-102](../issues/OPS.md#p-102)).
...
```
 ...
 cron */5 --notify-sweep→ bot_notify_sweep()                                              │
                      ...
                      │      пороги 7d/3d/1d/1h/10m (гейты по period_days.dat)             │
                      │      дедуп: notify_state.dat  (user|end_ts|label)                  │
                      │      → notify_user()  ── ⭐ «скоро закончится» + кнопки продления   │
                      │                                                                   │
 ...
 канала                │      chandm_topic_set(user_id, dm_chat_id, topic_id)              │
                      ...
                      │                                                                   │
 notify_user(user,txt, │    tg_send(chat, txt, kb)                   (личка, с кнопками)   │
   kb, channel_cta)   │      └─ не принято? → chandm_send(chat, txt, cta)  (канал, ссылкой)│
                      └───────────────────────────────────────────────────────────────────┘
                                        chandm_map.dat  ← общий файл (chmod 644) →
                      ┌─────────────────────────── надстройка ────────────────────────────┐
 зачисление/списание   │  денежная транзакция закоммичена                                  │
                      │    └─ уведомление о движении баланса:                              │
                      │           ├─ свой бот → личка (💵/🪙 …)                             │
                      │           └─ читает chandm_map.dat → «от лица канала»               │
                      └───────────────────────────────────────────────────────────────────┘
```
...
| Файл | Что там |
|---|---|
| `lib/notify.sh` | **вся новая логика**: `CE_MAP`/`ce()`, `fmt_date_dmy`, `bot_username`, `period_days_set/get`, `notify_user`, `chandm_send`, `chandm_topic_set`, `miniapp_screen_url`, `bot_renew_kb`, `bot_renew_link`, `bot_notify_activated`, `bot_notify_sweep` |
...
| `bot.conf` | `KEY=VALUE` | `BOT_TOKEN`, `BOT_USERNAME` (кэш getMe), `ADMIN_IDS`, `MINIAPP_URL` (§6.2) |
...
### Пометка «в бота»
Канальное сообщение получает суффикс со **ссылкой** (кнопок в директе канала
нет): по умолчанию `💬 Управляйте подпиской в боте: https://t.me/<username>`,
а если зовущий передал свой призыв — его (у напоминаний это ссылка на продление,
§6.2). Личка бота такой пометки не имеет — она и так «в боте», там кнопки.
...
| label | секунд | как пишем в тексте | гейт (длина периода) |
|---|---|---|---|
| 7d | 604800 | 7 дней | `period_days > 8` |
| 3d | 259200 | 3 дня | `period_days >= 3` |
| 1d | 86400 | 24 часа | `period_days > 2` |
| 1h | 3600 | 1 час | — |
| 10m | 600 | 10 минут | — |

**Остаток пишется словами порога, а не числом.** `end_ts` — это `23:59:59`, и в
момент отправки до порога всегда не хватает секунд: пересчитанный остаток давал
«23ч 59м» там, где человек ждёт «24 часа» ([issues/OPS.md#p-103](../issues/OPS.md#p-103)).
`format_remaining` (для TUI и `/status`) по той же причине округляет секунды
вверх до минуты и не печатает нулевые части.

Порогов пять, а не семь: «за 12 часов» между сутками и часом ничего не решал —
человек либо продлевает по суточному напоминанию, либо в последний час. «За 30
минут» заменено на «за 10 минут»: за полчаса до конца оплатить ещё успеют по
часовому, а десять минут — это уже «прямо сейчас».
...
**Частота:** cron `*/5 * * * * --notify-sweep`. Пороги 10мин/1ч ловятся только при
частом прогоне — не ставьте реже 5 минут (иначе «за 10 минут» проскочит мимо).
...
| многошаговая операция (оплата) для админа | надстройка | одно сообщение, шаги с галочками — приём описан в её документации |
...
## 6.2. Куда вести из уведомления

Команды в тексте (`/buy`) больше нет: у бота без модуля `sales` её не существует,
а в директе канала команду вообще не наберёшь. Вместо неё — прямой переход, свой
для каждого канала доставки:

| Канал | Что даём | Чем строится |
|---|---|---|
| личка бота | кнопки «продлить» и «пополнить» (ведут в мини-апп) | `bot_renew_kb` |
| директ канала | ссылка `https://t.me/<бот>?start=tariffs` | `bot_renew_link` |

Почему в канал ссылка на `/start`, а не сразу на мини-апп: прямая ссылка на
мини-апп из чата прямых сообщений канала не открывается (баг Telegram). Бот, приняв
`/start tariffs` (или `balance`), передаёт параметр надстройке, а та шлёт
приветствие с кнопкой на нужный экран.

Адрес мини-аппа бот берёт из `MINIAPP_URL` в `bot.conf` (вид
`https://t.me/<бот>/<приложение>`), добавляя `?startapp=<экран>`. Ключ не задан —
кнопок нет; если при этом бот продаёт сам (модуль `sales`), под сообщением
появится его собственная витрина тарифов.
...
- **Новый тип уведомления (баланс/оплата):** это сторона надстройки — шлётся
  после коммита денежной транзакции, best-effort (не ломать деньги отправкой).
  Как именно — в её документации.
...
- **Другой канал доставки** (напр. email): ветка в `notify_user` (bash) и парная
  правка на стороне надстройки.
...
- **Глобальный выключатель уведомлений на стороне надстройки** — если деньги
  движутся, а сообщений нет, проверять надо там, а не в менеджере.
