...
| Автоотключение по истечении | менеджер `lib/tgbot_daemon.sh` `bot_notify_expired` | `check_expired_users` (НЕ для тех, кто на бесплатном тарифе, см. issues P-52) | 🚫 |
| Бесплатный тариф: перевод, «трафик кончается», исчерпан, сброс окна | менеджер `lib/tgbot_daemon.sh` `bot_notify_free_*` | `freeplan_tick` (раз в минуту) | 🆓 ⚠️ ⛔ |
| Автопродление не прошло (не хватило баланса) | **надстройка** `TelegramNotifier::autorenewNoMoney` | `autorenew:run`, последний день подписки | ❗️ |
| Живой лог операции для АДМИНА (одно редактируемое сообщение) | **надстройка** `AdminLog` | создание счёта → шаги оплаты → зачисление | 💳 🛒 |
...
**Отрицательные дни ≠ продление.** `bot_extend_user` умеет укорачивать срок (эскроу
подарочной ссылки в мини-аппе). В этом случае ни `bot_notify_activated`, ни
`period_days_set` не вызываются: «Подписка активна» на снятие дней — ложь, а
отрицательная длина периода глушила гейты порогов и оставляла клиента вообще без
напоминаний (issues P-53).
...
## 6.1. Экономия сообщений: править, а не слать

Правило бота: **если у события уже есть сообщение на экране — его правят, а не
пишут рядом новое.**

| Приём | Где | Чем делать |
|---|---|---|
| навигация меню (клиент и админ-панель) живёт в одном сообщении | `lib/tgbot_daemon.sh` (обработка callback) | `bot_show chat mid text kb` — edit, а send только если правка не прошла |
| действие без своего экрана (кик, продление, «проверить оплату») | там же | всплывашка: положить текст в `CB_TEXT`, ответ уходит один раз в конце обработчика |
| многошаговая операция (оплата) для админа | надстройка | `AdminLog::step` — одно сообщение, шаги с галочками (docs/ADMIN-OP-LOG.md) |

Почему `answerCallbackQuery` один раз: Telegram принимает ответ на конкретное
нажатие ровно однажды — второй вызов молча пропадает вместе с текстом.

Чего НЕ схлопывать: уведомления о деньгах и о сроке. Их читают как чек, и
исчезнувшая (переписанная) строка выглядит как пропавший платёж.
...
- **Прежде чем добавлять уведомление — проверьте, не станет ли оно шагом уже
  существующего сообщения** (§6.1). Что ещё стоит уведомлять и в каком виде —
  [design/NOTIFY-ROADMAP.md](../design/NOTIFY-ROADMAP.md).
