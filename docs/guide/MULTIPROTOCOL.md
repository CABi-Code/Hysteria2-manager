...
* **Онлайн** сливается в `refresh_online`: Hysteria `/online` + online-статистика
  ...
  `sourceIP` (`proto_tuic_activity_lines conns`), потому что `metadata.user`
  sing-box не отдаёт. См. P-17.
...
1. **Побайтный трафик TUIC** (sing-box) сейчас НЕ считается вовсе
   (`proto_collect_tuic_traffic`, раз в минуту): у sing-box 1.13 нет StatsService
   (`v2ray_api` убрали в 1.8), поэтому суммируются дельты байт соединений из
   `clash_api /connections` — но соединения фильтруются по `.metadata.user`,
   которого sing-box не отдаёт (проверено на 1.13.14: 44 соединения подряд, поля
   нет ни у одного). Резолв по `sourceIP`, как для онлайна, в квоты не пускаем:
   на общем CGNAT-IP байты запишутся не тому юзеру. Точный per-user учёт возможен
   через per-IP nftables-счётчики на порту TUIC (см. паритет лимитов,
   docs/guide/SPEED-LIMITS.md). Hysteria + VLESS/SS — точно. См. P-02/P-17.
