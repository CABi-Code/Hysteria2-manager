...
| [SUB-ALERTS.md](SUB-ALERTS.md) | **Чужое устройство в подписке**: когда бот пишет владельцу и почему чаще молчит — четыре правила против спама и ложных срабатываний, кнопка сброса ссылки | `lib/notify.sh`, `hy2-manager.sh` |
| [SUB-CLIENTS.md](SUB-CLIENTS.md) | **Чем пользуется человек**: приложение, версия, ОС и модель устройства из заголовков запроса за подпиской; почему на туннеле этого не видно ни одному движку; формат `subapps.dat` | `lib/ip_tracking.sh`, `lib/config.sh`, `webapi/wa_payload.py` |
...
| [SUB-BROWSER.md](SUB-BROWSER.md) | **Ссылку-подписку открыли браузером**: вместо простыни base64 — редирект на страницу подписки в кабинете; как браузер отличается от клиента (`Sec-Fetch-Mode`), настройка `SUB_WEB_URL`, ручка «чей это токен» | `lib/node.sh`, `lib/caddy.sh`, `lib/ui_subscription.sh`, `webapi/wa_payload.py` |
...
| [CLUSTER-CHANNELS.md](CLUSTER-CHANNELS.md) | **Каналы связи между нодами**: файловый pull (`/cluster/*`, статика Caddy) против Web API (push «сделай сейчас»), что по какому ходит и почему, как добавить раздел, модель угроз при взломе канала и что ещё стоит перенести | `lib/cluster.sh`, `lib/publish.sh`, `lib/caddy.sh`, `webapi/*` |
